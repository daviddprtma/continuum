...
export const relayService = {
  async createRelay(
    ...
    try {
      ...
      if (!senderAddress || !receiverAddress || !amount || amount <= 0) {
        console.error("Invalid relay data:", {
          senderAddress,
          receiverAddress,
          amount,
        });
      ...
      if (!data) {
        console.error("No data returned from relay creation");
        return null;
      }
  ...
  async getRelays(walletAddress: string): Promise<Relay[]> {
    try {
      ...
      const { data, error } = await supabase
        ...
        .select("*")
        .or(
          `sender_address.eq.${walletAddress.toLowerCase()},receiver_address.eq.${walletAddress.toLowerCase()}`,
        )
        .order("created_at", { ascending: false });
  ...
  async expireOldRelays(): Promise<void> {
    try {
      const { error } = await supabase.rpc("expire_old_relays");
      if (error) {
        console.error("Error expiring old relays:", error);
      }
    } catch (error) {
      console.error("Error in expireOldRelays:", error);
    }
  },
};
...
export const legacyService = {
  ...
  async addBeneficiary(
    ...
    try {
      ...
      if (planError || !legacyPlan) {
        console.error(
          "Error fetching legacy plan for beneficiary addition:",
          planError,
        );
        return null;
      }
  ...
  async getBeneficiaries(legacyPlanId: string): Promise<Beneficiary[]> {
    try {
      ...
      if (planError || !legacyPlan) {
        console.error(
          "Error fetching legacy plan for beneficiaries:",
          planError,
        );
        return [];
      }
      ...
      // Transform database format to component format
      const transformedBeneficiaries: Beneficiary[] = (data || []).map(
        (beneficiary) => ({
          id: beneficiary.id,
  ...
  async deleteBeneficiary(beneficiaryId: string): Promise<boolean> {
    try {
      ...
      if (beneficiaryError || !beneficiary) {
        console.error(
          "Error fetching beneficiary for deletion:",
          beneficiaryError,
        );
        return false;
      }
  ...
  async updateBeneficiary(
    ...
    try {
      ...
      if (beneficiaryError || !beneficiary) {
        console.error(
          "Error fetching beneficiary for update:",
          beneficiaryError,
        );
        return null;
      }
