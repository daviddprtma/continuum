...
const SetBeneficiariesForm = lazy(
  () => import("../components/SetBeneficiariesForm"),
);
const BeneficiariesDisplay = lazy(
  () => import("../components/BeneficiariesDisplay"),
);
const EditBeneficiaryModal = lazy(
  () => import("../components/EditBeneficiaryModal"),
);
...
const LegacyPage: React.FC = () => {
  ...
  const [showHeartbeatConfirmation, setShowHeartbeatConfirmation] =
    useState(false);
  const [pendingDeleteBeneficiaryId, setPendingDeleteBeneficiaryId] = useState<
    string | null
  >(null);
  const [pendingMomentConfig, setPendingMomentConfig] =
    useState<LegacyMoment | null>(null);
  ...
  const currentTotalPercentage = beneficiaries.reduce(
  ...
  const loadLegacyData = async () => {
    ...
    try {
      ...
      const userData =
        await walletAccountService.getWalletAccount(hederaAccountId);
      if (userData) {
  ...
  const handleAddBeneficiary = async (
    ...
    if (newTotal > 100) {
      ...
      setToastMessage(
        `Total percentage cannot exceed 100%. You can allocate up to ${remainingPercentage.toFixed(1)}% more.`,
      );
      setToastType("error");
      setShowToast(true);
      return;
    ...
    try {
      ...
      if (!currentLegacyPlanId) {
        ...
        const newPlan = await legacyService.createOrUpdateLegacyPlan(
          hederaAccountId,
          defaultMoment,
        );
        if (!newPlan) {
      ...
      const newBeneficiary = await legacyService.addBeneficiary(
        currentLegacyPlanId,
        beneficiaryData,
      );
      if (newBeneficiary) {
  ...
  const confirmSetMoment = async () => {
    ...
    try {
      const updatedPlan = await legacyService.createOrUpdateLegacyPlan(
        hederaAccountId,
        pendingMomentConfig,
      );
      if (updatedPlan) {
  ...
  const handleUpdateBeneficiary = async (
    ...
    try {
      const updatedBeneficiary = await legacyService.updateBeneficiary(
        beneficiaryId,
        beneficiaryData,
      );
  ...
  const confirmDeleteBeneficiary = async () => {
    ...
    try {
      const success = await legacyService.deleteBeneficiary(
        pendingDeleteBeneficiaryId,
      );
      if (success) {
        setBeneficiaries((prev) =>
          prev.filter((b) => b.id !== pendingDeleteBeneficiaryId),
        );
  ...
  const confirmHeartbeat = async () => {
    ...
    try {
      const updatedUser = await walletAccountService.upsertWalletAccount(
        hederaAccountId,
        chainId,
      );
  ...
  const beneficiaryToDelete = beneficiaries.find(
    (b) => b.id === pendingDeleteBeneficiaryId,
  );
  ...
  return (
    <div className="min-h-screen relative flex flex-col bg-[#F8F8F8]">
      <Suspense
        fallback={
          <div className="h-16 bg-white shadow-sm w-full animate-pulse" />
        }
      >
        <AestheticNavbar
          leftLinkPath="/stream"
              ...
              <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
                <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-12 shadow-xl max-w-md">
                  ...
                  <p className="text-gray-600 mb-8 leading-relaxed">
                    Build a trustless legacy that activates when the time is
                    right. No lawyers, no paperwork—just secure, automated
                    distribution.
                  </p>
                  <button
                    onClick={() => open()}
      ...
      <Suspense fallback={null}>
        ...
        <ConfirmationDialog
          ...
          message={
            beneficiaryToDelete
              ? `Are you sure you want to delete "${beneficiaryToDelete.name}"? This action cannot be undone and will remove them from your legacy plan.`
